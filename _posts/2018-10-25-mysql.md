...
* 左外连接（LEFT OUTER）
    * 左表为基础
    * 一种是A表所有行拼接B表列内容，数据不存在则为NULL`SELECT <list> FROM tablea a LEFT JOIN tableb b ON a.key = b.key`
    * 一种是A表和B表交集行的列的拼接，和INNER JOIN相同`SELECT <list> FROM tablea a LEFT JOIN tableb b ON a.key = b.key WHERE b.key is not NULL`
* 右外连接（RIGHT OUTER）
    * 右表为基础
    * 一种是B表所有行拼接A表列的内容，数据不存在则为NULL`SELECT <list> FROM tablea a RIGHT JOIN tableb b ON a.key = b.key`
    * 一种是B表和A表交集行的列的拼接，和INNER JOIN相同`SELECT <list> FROM tablea a RIGHT JOIN tableb b ON a.key = b.key WHERE a.key is not NULL`
* 全外连接（FULL OUTER）
    * MySQL不支持
    * `SELECT <list> FROM tablea a LEFT JOIN tableb b ON a.key = a.key UNION ALL SELECT <list> FROM tablea a RIGHT JOIN tableb b ON a.key = b.key`
* 交叉连接（CROSS）
    * 笛卡尔积，或者理解为多重循环`SELECT <list> FROM tablea a CROSS JOIN tableb b`
* 部分技巧
    * 如何更新使用过滤条件（FROM子句）中包括自身的表：不可以使用`UPDATE table1 SET column='new' WHERE table1.column2 IN (SELECT b.column2 FROM table1 INNER JOIN table2 b on a.column2 = b.column2)`，应该写为`UPDATE table1 a JOIN (SELECT b.column2 FROM table1 a INNER JOIN table2 b ON a.column2 = b.column2) b ON a.column2 = b.column2 SET a.column1='new'`
    * JOIN优化子查询：`SELECT a.column1, a.column2, (SELECT column2 from table2 b WHERE a.column1 = b.column1) AS column3 FROM table1 a`，这样写每一行数据会查询一次，应该改为`SELECT a.column1, a.column2, b.column2 AS column3 FROM table1 a LEFT JOIN table2 b ON a.column1 = b.column1`
    * 使用JOIN优化聚合子查询：`SELECT a.column1, b.column2, b.column3 FROM talbe1 a JOIN table2 b ON a.id = b.column1 WHERE b.column3=(SELECT MAX(c.column3) FROM table2 c WHERE c.column2=b.column2)`，这方式效率低，使用`SELECT a.column1, b.column2, b.column3 FROM table1 a JOIN table2 b ON a.id = b.column1 JOIN table2 c ON c.column1 = b.column1 GROUP BY a.column1, b.column2, b.column3 HAVING b.column3 = MAX(c.column3)`
    * 分组选择数据：`SELECT a.column1, b.column2, b.column3 FROM table a JOIN table b ON a.id=b.column1 WHERE column1='value' ORDER BY b.column3 DESC LIMIT2`，单个查询次数多，使用`SELECT d.column1, c.column2, column3 FROM (SELECT column1, column2, column3, (SELECT COUNT(*) FROM table2 b WHERE b.column1=a.column1 AND a.column3<=b.column3) AS cnt FROM table2 a GROUP BY column1, column2, column3) c JOIN table1 d ON c.column1=d.id WHERE cnt <= 2`

#### 其他技巧
* 如何进行行列转换（行转列：聚合查询后行转列用于查询，列转行：一个Column存储多条数据分离）
    * CASE方式行转列`SELECT SUM(CASE WHEN column='value1' then column3 END) AS 'value1', SUM(CASE WHEN column='value2' then column3 END) AS 'value2' FROM table1 a JOIN table2 b ON a.id = b.column1`
    * UNION方式实现列转行`SELECT column1, 'value1' as column2, column3 FROM table1 a JOIN table2 b ON a.id=b.cloumn1 UNION ALL SELECT column1, 'value2' as column2, column3 FROM table1 a JOIN table2 b ON a.id=b.column1`
    * 使用序列化表的方式实现行转列`SELECT column1 case WHEN c.id=1 THEN 'value1' WHEN c.id=2 then 'value2', c.id=3 then 'value3' END AS column2, coalesce(CASE WHEN c.id=1 THEN value1 END, CASE WHEN c.id=2 THEN value2 END, CASE WHEN c.id=3 THEN value3 END) AS column3 FROM table1 a JOIN table2 b ON a.id=b.column1 CROSS JOIN table3 c WHERE c.id <= 3 ORDER BY column1`
* 数据库中唯一序列号的常用方式
    * 优先选择系统提供的序列号生成方式`CREATE TABLE t (id int auto_imcrement not null, primary_key (id))`，注意事务建立数据后回滚会出现序号空档
    * 使用SQL存储过程
* 如何处理重复数据
    * 判断是否重复：`SELECT column1, count(*) FROM table GROUP BY column1 HAVING COUNT(*) > 1`
    * 删除重复数据，相同数据保留ID最大的：`DELETE a FROM table a JOIN(SELECT column1, COUNT(*), MAX(id) AS id FROM table GROUP BY column1 HAVING COUNT(*)>1) b ON a.column1 = b.column1 WHERE a.id < b.id`
* 在子查询中匹配两个值
    * MySQL多列过滤方式：`SELECT a.column1, b.column2, b.column3 FROM table1 a JOIN table2 b ON a.id=b.column1 WHERE (b.column1, b.column3) IN (SELECT column1, MAX(column3) FROM table2 GROUP BY column1)`
* 解决同一属性多值过滤
    * JOIN方式：`SELECT a.column1, b.column2, c.column2 FROM table1 a JOIN table2 b ON a.id=b.column1 and b.column2='value1' JOIN table2 c on c.column1=b.column1 AND c.skill='value2' WHERE b.column3 > 0 and c.column3 > 0`
    * 使用GROUP BY：`SELECT a.column1 FROM table1 a JOIN table2 b ON a.id=b.column1 WHERE b.column2 IN ('value1', 'value2', 'value3', 'value4') AND b.column3 > 0 GROUP BY a.column1 HAVING COUNT(*) >= 2`
