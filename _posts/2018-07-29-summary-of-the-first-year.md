...
自己想方案满足需求本来没有问题，但是信息量的差距会让工作上重重受阻，这是我在一年间遇到的最大的问题，感觉好多时间被这种问题消耗，而领导不在意这种情况，反馈无效，感觉无解

二是资源利用的问题

当年我来工作的时候，组里一共6个人，一个我，一个上届应届生，其他都是腾讯呆了5年以上的人，公司整个的技术部门绝大多数都是从腾讯挖来的，本来想着可以来好好学习

但实际工作中，导师们相当于派活的角色，就是上面的一句话需求，当自己遇到难题，导师们并往往不提供解决方案，全靠自己学习解决

工作中基本没有合作，不管小组还是和别的部门，本来有些系统和事情可以让专门负责的部门来做，但是出于各种不愿共享信息的理由，或者导师们为了增多绩效的目标数量和抢活，也做了很多无意义的工作

比如很多东西只有一期，能跑就行，不要文档和优化，往往负责人离职就要重新做，事实上我已经改了几个之前离职的人做的项目，基本上和重做一样

比如整个公司内技术的基础建设完全用不到，什么语言、数据库、日志系统、MQ都要预研、自研，一个组，每人做两三个项目，都用不同技术，产品、开发、测试、运维全是自己，而很多时候，大家做的事情一样，比如解析IP，基本上每个人要解析一次，而不是一个人解析好大家从他那里提供的接口获取，有大量重复的工作

再比如类似工单系统，开发有自己的工单系统，安全也要重新做一个维护，就强行为了绩效了，实际上完全可以融入开发的系统，让他们系统的维护者将我们的漏洞工单作为bug单一样用

此类问题也是很无解，大家都觉得项目能上线能跑能稳定就好，不在意这些问题
...
### 渗透测试思考
我的理解，渗透本身就是安全运营中的一环，可以是SDL建设中的一部分检查，渗透之所以重要，是因为在有限的成本之下，渗透是最能有效防止安全问题的措施

渗透是一个低成本的工作，在多个系统交互的项目中，往往不需要项目人员的配合，给个入口就可以开始工作了，而能力差距不大的情况下，检查到的点和往往和真正的攻击者相差不大，也就是覆盖面广，低成本高覆盖是渗透的极大优势

大公司安全建设初期和小公司投入到安全的成本不多时渗透应该是最有效的方式，能覆盖掉SQL、越权、XXE、SSRF、一部分XSS、逻辑就很有效了，之后安全运营和SDL等建设起来，需要的成本变大了，渗透的作用最好转移到越权等逻辑相关等漏洞上，解放人力，花在业务逻辑的安全性检查上

低廉的成本中不求完全没有问题，但是希望高风险能够完全覆盖
...
### 安全运营思考
我个人理解，运营是一件繁琐的活，每天扫描报告一定要看，渗透每次要认真做，漏洞工单要认真查证，都只能硬着头皮做下去

已有的能力认真硬着头皮做，需要建设的能力慢慢补起来，把事情做到制度化标准化，让别人都上了安全运营的船再慢慢优化和迭代

以我外网扫描为例：IP端口扫描（nmap），主机扫描（Nessus），API扫描（Burp Suite），这些扫描都是我来以后从无到有都建设，从一开始随便找台机器跑脚本，到开始做扫描任务管理系统定时扫描，到做代理扫描器给从测试环境收集API给Burp Suite扫描，先要有，然后再优化，不管规则好不好，误报漏报多不多，要每天硬着头皮一项项看，整理整理规则，再做优化

之后可能会加上管理台扫描，弱密码爆破，通用组件漏洞的PoC扫描和输出给业务调用等一些新能力，做已有的系统的时候也要好好设计，考虑可以复用的地方，都可以慢慢迭代下去

#### 技术栈
如果知识是教你怎么做，技术栈就是行动力

感觉我们这一行行动力很重要，不是什么技术和框架我们都研究过的，但是有漏洞，还是要搭建环境去验证，业务的代码是乱七八糟的，还是要按每一项检查点排除万难去试，fuzz不就是靠行动力去一条条试吗

现在我是以Docker+Git+Python+Golang+MySQL+Redis+RabbitMQ这样的技术栈，Golang和Python开发一些工具，Flask做扫描管理台等一些Web开发，偶尔JAVA写Burp Suite的插件，以后如果能开发Agent收集流量和日志打算学习Elasticsearch Stack或者Solr

以前觉得写程序Copy and Paste From Github、Stackoverflow，能弄明白代码逻辑，写出来没问题就行，现在越来越觉得为了效率，熟练很重要，用自己顺手的技术栈，要熟练它，遇到问题能很快写出程序，很快自动化，才能提高执行力

写收集API的代理的时候，参考了[goproxy](https://github.com/elazarl/goproxy)，因为他对HTTPS证书签名导致HTTPS连接慢对问题，我改写了它，把签名后证书用Hash表存起来，但是改写过程中发现对它对设计不明白，很多地方地方设计不懂，于是自己参考它的逻辑重新写了一遍，很多时候觉得某个地方的功能设计用不上不必要，写完了使用的时候觉得这里又是很有必要的然后加了上去，反复几次，发现最后实现的功能基本一致

可能这就是经验吧，对某个工具和系统有使用，总结什么是必要的什么是不必要的，才能更好实现程序，在有经验之前，多参考很重要，有了经验之后，执行力很重要

### 总结
应届生校招，选择一个觉得问问题问得有水平的面试官的公司，其实是我当年对道对追求太高了，年轻人还是多注重术，注重技巧，注重积累，培养执行力
